...
// $('#textAreaInformation').toggle('slow');
    ...
    $.get('http://'+serverIpObj[server].ip+'/id.txt', function( serverName ) {
   ...
   } else {alert('Please Choose Server And Brand');}
...
// $('#textAreaInformation').toggle('slow');
    ...
    //   $('#textAreaInformation').hide('slow');
    //   $('#textAreaInformation').show('slow');
    //   $('#textAreaInformation').val('Time:'+dateString+'\nServer Version:null Brand: '+brandName+'\nPsps: '+createResultFromServer(resultPsps)+'\nLanguages:'+langISO(createResultFromServer(resultLanguages))+'\nForex:'+resultForex+ ' \nRegulation:'+resultRegulation+' ' );
...
function addPsp() {
  ...
  }else{ alert('Please Enter Psp Name And ID'); }
}
...
function queryMakerApp(array1) {
    ...
    cmdCommand(' (winscp.com /command "open sftp://' + USERNAME + ':' + PASSWORD + '@' + serverIpObj[server].ip  + '\" \"call echo ' + bonusNoWager + ' > sql2.sql\"  \"call echo ' + bonusNoWager2 + '  >> sql2.sql\"  \"call sudo chmod 777 sql2.sql && mysql optionfair_trading < sql2.sql \" \)\;', serverName);
    cmdCommand3(' (winscp.com /command "open sftp://' + USERNAME + ':' + PASSWORD + '@' + serverIpObj[server].ip  + '\" \"call echo ' + sql3 + ' > sql3.sql\" \"call echo ' + messageTemplateOnly + ' > messageTemplateOnly.sql\"  \"call sudo chmod 777 sql3.sql && mysql optionfair_trading < sql3.sql \" \"call sudo chmod 777 messageTemplateOnly.sql && mysql optionfair_trading < messageTemplateOnly.sql \" \)\;', serverName);
    ...
    function queryMaker2(pspArr) {
        ...
        if      (addForex  ===  true  && addBinary ===  false && sortInstrumentsCheckbox === false)               {sql3.push('delete from brand_platform_rel  where brand_id=' + brands[brandName].id + '\\; INSERT INTO brand_platform_rel \\(platform_id, brand_id, sort\\) VALUES \\(81,' + brands[brandName].id + ',0\\)\\;');}

        else if (addBinary ===  true  && addForex  ===  false && sortInstrumentsCheckbox === false)               {sql3.push('delete from brand_platform_rel  where brand_id=' + brands[brandName].id + '\\; INSERT INTO brand_platform_rel \\(platform_id, brand_id, sort\\) VALUES \\(80,' + brands[brandName].id + ',0\\)\\;');}

        else if (addForex  ===  true  && addBinary ===  true && sortInstrumentsCheckbox === false)  {sql3.push('delete from brand_platform_rel  where brand_id=' + brands[brandName].id + '\\; INSERT INTO brand_platform_rel \\(platform_id, brand_id, sort\\) VALUES \\(80,' + brands[brandName].id + ',0\\),\\(81,' + brands[brandName].id + ',1\\)\\;');}
